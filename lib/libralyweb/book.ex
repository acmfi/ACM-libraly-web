defmodule Libralyweb.Book do
  ...
  def get_book(id) do
    case String.match?(
      ...
      true ->
        case Books
             |> Libralyweb.Repo.get_by(ISBN: id |> String.replace(~r/(-| )/, "") |> String.upcase()) do
          nil ->
            {:ok, %{}}
      ...
      false ->
        query =
          from(b in Libralyweb.Models.Books,
            # Simple regex for searching multiple books
            where: ilike(b.title, ^"#{id}%")
          )

        {:ok, Libralyweb.Repo.all(query)}
    end
  end

  def get_books(cursor, filters, limit \\ 50) do
    base_query = from(Libralyweb.Models.Books, as: :book, order_by: :inserted_at)
    ...
    %{entries: entries, metadata: metadata} =
      Libralyweb.Repo.paginate(query, after: cursor, cursor_fields: [:inserted_at], limit: limit)
